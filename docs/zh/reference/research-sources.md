...
Agent Tool v0.1.0 参考了成熟的工具、API、事件、遥测、授权与 agent 协议。这些参考影响标准形状，但不转移 Agent Tool 的语义所有权。

| 来源 | Agent Tool 借鉴内容 |
| --- | --- |
| [Model Context Protocol tools specification](https://modelcontextprotocol.io/specification/2025-06-18/server/tools) | Tool discovery、`tools/list`、`tools/call`、`inputSchema`、`outputSchema`、content blocks、`structuredContent`、`resource_link`、embedded resources、annotations、errors 与 safety guidance。 |
...
| [OpenAI function calling guide](https://platform.openai.com/docs/guides/function-calling) | Function tools、tool calls、tool outputs、`tool_call_id`、tool choice、allowed tools、strict schema mode、custom tools 与 tool search。 |
...
| [OpenTelemetry GenAI MCP conventions](https://opentelemetry.io/docs/specs/semconv/gen-ai/mcp/) | `execute_tool` spans、GenAI tool call attributes、JSON-RPC ids、MCP session ids 与 trace/span correlation。 |
